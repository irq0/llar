...
(deftest runtime-docs-come-from-var-metadata
  (let [construct-names (set (map :symbol (uut/runtime-constructs)))]
    ...
    (is (contains? construct-names 'reader-favorites))
    (is (not (contains? construct-names 'reader-favorite)))
    ...
    (is (contains? construct-names 'digest))
    (is (contains? construct-names 'podcast-download))
