   ...
   [:script
    (string/join " "
      ["$(\"body\").keypress(function(event) {"
       ...
       "    if (event.key == \"n\") { "
       "      window.location.href = $(\"#btn-next-item\").attr(\"href\");"
       "    } else if (event.key == \"p\") {"
       "      window.history.back();"
       "    } else if (event.key == \"N\") {"
       ...
       "      console.log([\"regular\", event, content]);"
       "      event.preventDefault();"
       "      var items = $(\".item-content-body\").find(\"h1, h2, h3, h4, h5, h6, h7, p, section, pre, ul, blockquote, img\");"
