(ns llar.config
  (:require
   ...
   [llar.fetch :refer [make-item-hash] :as fetch]
   [llar.fetch.custom]
   [llar.fetch.feed]
   [llar.fetch.http]
   [llar.fetch.imap]
   ...
   [llar.fetch.reddit]
   [llar.fetch.twitter]
   [llar.fetchutils :refer [make-category-filter-deny
...
(fetch-reddit (src/reddit "Clojure" :hot) :min-score 10
              :options #{:mark-read-on-view}
              :tags #{:reddit :tech})
                  ...
                  (fn [item]
                    (let [article (S/select
                          ...
                          html (-> {:type :element
                                    ...
                                    :tag :article
                                    :content article}
                                   llar.http/sanitize hickory-to-html)]
